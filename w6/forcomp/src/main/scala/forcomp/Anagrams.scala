...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.groupBy(_.toChar).map(el => (el._1, el._2.size)).toList.sortBy(_._1)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def iter(occ: Occurrences): List[Sentence] = occ match {
      case Nil => List(List())
      case occ => for {
        subs <- combinations(occ)
        word <- dictionaryByOccurrences(subs)
        sentence <- iter(subtract(occ, subs))
      } yield word :: sentence
    }
