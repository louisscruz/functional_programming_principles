...
abstract class TweetSet {
   ...
   *
   * Question: Should we impelment this method here, or should it remain abstract
   * and be implemented in the subclasses?
   */
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tweet => google.exists(el => tweet.text.contains(el)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(tweet => apple.exists(el => tweet.text.contains(el)))
